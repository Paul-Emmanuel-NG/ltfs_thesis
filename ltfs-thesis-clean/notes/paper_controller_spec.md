...
- Express-layer speed-factor bonus exists as an optional feature and can be applied to admitted vehicles while on express edges.
...
- In the active Paper 1 runtime path, gate mode is TST-focused / TST-enforced.
- If UWA is requested in matrix/env settings, this does not imply active UWA execution in the current runtime path.
- Any UWA-requested run must be treated as an **explicit ablation request**, not the default Paper 1 controller.
...
- active Paper 1 runtime path is TST-focused / TST-enforced; UWA requests in matrix/env do not imply active UWA execution
